...
<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; margin: 0; padding: 0;">
    <div style="max-width: 600px; margin: 20px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
        ...
        <div style="padding: 20px; color: #333;">
            ...
            <a href="{{ route('customer.auth.password.reset', ['token' => $data['token'], 'email' => $data['email']]) }}" 
               style="display: inline-block; margin: 20px 0; padding: 10px 20px; background-color: #007BFF; color: #ffffff; text-decoration: none; border-radius: 5px; font-size: 14px;">Reset Password</a>
            ...
            <p style="margin: 10px 0; line-height: 1.6;">
                <a href="{{ route('customer.auth.password.reset', ['token' => $data['token'], 'email' => $data['email']]) }}" 
                   style="color: #007BFF; text-decoration: none;">{{ route('customer.auth.password.reset', ['token' => $data['token'], 'email' => $data['email']]) }}</a>
            </p>
        ...
        <div style="text-align: center; padding: 15px; background-color: #f4f4f4; font-size: 12px; color: #888;">
            ...
            <p style="margin: 0;"><strong>Tickies</strong></p>
        </div>
    </div>
</body>
</html>
